...
while getopts "hs:d:m:n:b:xw:v" opt; do
    case "$opt" in
    h)
        cat << EOF
requirement: 
...
usage: -s server_ssh_destination -d directory_in_deployment_server -m mountpoint_directory [-n name_of_remote_git] [-b branch_target] [-w wait_seconds] [-v]
...
sshfs -o allow_other $server_ssh_destination:$directory_in_deployment_server "$mountpoint_directory"
